...
pub fn is_prime(sieve: enums::SieveMethod, num: u64) -> bool {
    // If we don't have a list of primes yet, find a list of primes to evaluate against first.
    let _list_of_primes: Vec<u64> = list_primes(sieve, (num as f64).sqrt() as u64, None);

    return is_prime_with_known_primes(num, &_list_of_primes);
}

/// Private function
/// This actually do the work of checking primes.
/// Mandatory: Give it a slice of SORTED prime numbers up to at least sqrt of num.
pub fn is_prime_with_known_primes(num: u64, primes: &[u64]) -> bool {
